...
DEFAULT_ALLOWED_KEYWORDS = [
    ...
    # Extra keywords to keep City of Greater Geelong (council) data in strict scope
    "geelong", "greater geelong", "city of greater geelong", "local government",
    "customer service", "civic", "infringement", "rates"
]
...
# Resolve "1"/"first"/names to a service id from last results or global meta
...
def _estimate_transit_minutes(distance_km: float, driving_minutes: Optional[float], walking_minutes: Optional[float]) -> float:
    ...
    # Clamp with reasonable bounds vs walking
...
# ------------------------
# RAG index over services
# ------------------------
def _combine_day_hours(df: pd.DataFrame, row: pd.Series) -> str:
    # builds "Mon 09:00–17:00; Tue 09:00–17:00; ..."
    def _day_col(day_long: str, day_short: str) -> Optional[str]:
        return pick_col(df, [day_long, day_short])
    days = [
        ("monday", "mon", "Mon"),
        ("tuesday", "tue", "Tue"),
        ("wednesday", "wed", "Wed"),
    ...
    parts = []
    for long_d, short_d, label in days:
        col = _day_col(long_d, short_d)
        if col and col in df.columns:
            val = str(row.get(col)).strip()
            if val and val.lower() not in {"nan", "none", "closed"}:
...
class ServiceIndex:
    ...
    def _load_csv(self, path: str) -> pd.DataFrame:
        # safer read for BOM/odd encodings
        df = pd.read_csv(path, encoding="utf-8-sig")
        return df

    def _to_docs(self, df: pd.DataFrame):
        name_col = pick_col(df, ["name", "service_name, office_name", "title"])
        desc_col = pick_col(df, ["description", "details", "about", "notes", "desc"])  # added 'desc'
        ...
        for i, row in df.iterrows():
            ...
            if not hours:
                # Build from per-day columns if present (Geelong CSV style)
                hours = _combine_day_hours(df, row)
            ...
            # If the CSV is a council dataset without explicit category, nudge towards 'Council Service'
            if not cat and ("geelong" in name.lower() or "geelong" in addr.lower()):
                cat = "Council Service"
...
def update_profile_tool(name: Optional[str] = None, language: Optional[str] = None, contact_channel: Optional[str] = None,
    ...
    safe_prof = {
        ...
        if k not in {"contact_value"}  # keep private fields minimal in responses
    }
...
- Be inclusive: short sentences, clear steps, respect user's language. Offer easy-read on request or when access needs are present.
...
def build_agent():
    ...
    # Render tool descriptions once and inject as partials
...
def main():
    ...
    if not os.path.exists(csv_path):
        print(f"[i] No dataset found at {csv_path}. Seeding a small demo CSV…")
        seed_sample_dataset(csv_path)
        print("[i] Replace with a real government dataset for production (e.g., Services Australia Service Centres, Legal Aid locations, Council community centres).")

    print(f"[i] Loading dataset: {csv_path}")
    try:
        INDEX = ServiceIndex(csv_path)
        print(f"[i] Loaded {len(INDEX.meta)} services.")
    ...
    print("======================================")
    print(" CommunityMate — DEMO MODE (no real bookings)")
    print(" Strictly for Australian government & community services")
    print("======================================")
    print("Type 'exit' to stop.")
    print("Try: 'I need food relief near Geelong' or 'Where is the nearest council customer service centre?'")
    print("Tip: 'Save my home as 123 Moorabool St, Geelong' to get map links and times from home.")
    print("Privacy: We'll ask your consent before any booking, forms, or reminders.")
    while True:
        try:
            msg = input("\nYou: ").strip()
